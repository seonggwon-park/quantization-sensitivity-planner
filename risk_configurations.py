...
def build_ranked_protection_configurations(
    layer_names: list[str],
    ranked_layers: list[str],
    protect_counts: list[int],
    ranking_label: str,
    ...
    """
    Build mixed-precision controls from any ranking metric.

    Examples:
    - ranking_label="oracle"
    - ranking_label="weight_l2"
    - ranking_label="activation_error"
    - ranking_label="forward_damage"
    """
    ...
    for count in unique_counts:
        ...
        configurations.append(
            {
                "name": (
                    f"{normalized_label}_top{count}_"
                    f"{protected_label}"
                ),
                "selection_policy": (
                    f"{normalized_label}_high_risk"
                ),
                "protected_layers": top_layers,
                "layer_bits": make_full_assignment(
                    layer_names=layer_names,
        ...
        if include_bottom_controls:
            ...
            configurations.append(
                {
                    "name": (
                        f"{normalized_label}_bottom{count}_"
                        f"{protected_label}"
                    ),
                    "selection_policy": (
                        f"{normalized_label}_low_risk_control"
                    ),
                    "protected_layers": bottom_layers,
                    "layer_bits": make_full_assignment(
                        layer_names=layer_names,
